"""
Evaluation helpers for the waste-bin fill-level models

The saved CNN reports accuracy, macro precision, macro recall, macro F1, and a
confusion matrix
"""
...
def _evaluate_predictions(y_test, y_pred, class_names):
    """Compute the classification metrics used in the report
...
def _cnn_class_score_rows(probabilities, class_names):
    """Convert softmax rows to class-score dictionaries
...
def evaluate_cnn_model(
    ...
    """Load the saved CNN and evaluate the held-out test split

    Postprocessing metrics use fixed thresholds on the CNN scores. The model is
    not retrained
    ...
    Returns:
        Metrics, split details, and postprocessing outputs
    """
    ...
    print(f"F1:        {result['macro_f1']:.4f} (Macro)")
    print("\nConfusion Matrix:")
    ...
    print("\n--- Postprocessing Risk-Score Policy ---")
    postprocessed = [
        postprocess_prediction(
