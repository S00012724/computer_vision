"""
Data and evaluation helpers for the HOG + Linear SVM comparison

The comparison model uses HOG features plus a Linear SVM, giving the report a
...
# Use a fixed 64 x 64 input so feature extraction stays fast and every image has
# the same descriptor length for the Linear SVM
...
def extract_hog_features(
    ...
    Parameters follow the course HOG layout: 8x8 cells, 16x16 blocks, 8-pixel
    block stride, and 9 orientation bins. The image is converted to grayscale
    because HOG measures intensity gradients
...
def _records_to_hog_features(records, img_size=HOG_IMAGE_SIZE):
    """Convert image records into HOG features and label arrays
...
def build_comparison_model():
    """Build the HOG + Linear SVM comparison model

    StandardScaler normalizes the HOG descriptor columns before SVM training.
    class_weight="balanced" gives more weight to the smaller needs_collection
    class. random_state=42 keeps the result reproducible
...
def evaluate_candidate(name, estimator, X_train, y_train, X_test, y_test, labels):
    """Train and score one model on the shared held-out split

    Metrics match the CNN evaluation: accuracy, macro precision, macro recall,
