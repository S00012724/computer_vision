...
def build_cnn(
    ...
    """Build the ResNet50 transfer classifier

    `include_top=False` removes the ImageNet classifier so this project can add
    a two-class head. The base stays frozen by default because the dataset is
    small. Dropout 0.3 reduces overfitting before softmax
    ...
    # Use Adam with sparse integer labels and report accuracy during training
    model.compile(
        optimizer="adam",
...
def train_and_evaluate_cnn(
    ...
    """Train the CNN, save it, and evaluate the test split

    Defaults match the reported run: 5 epochs, batch size 32, 96 x 96 images,
    and the shared stratified hold-out split from src.data
...
def print_cnn_results(result):
    """Print training and test metrics in report order
