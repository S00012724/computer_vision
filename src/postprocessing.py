"""
Utilities for collection-priority decisions

The CNN outputs class scores. The app turns them into one action: collect now,
leave the bin, or send the case to review. Every decision comes from one score,
P(needs_collection)
"""
...
# review because it is operationally uncertain
...
def _score_for(
    ...
    """Read one class score when it is present
...
def _needs_collection_score(
    ...
    """Return the needs_collection score used as risk

    Prefer the explicit needs_collection softmax score. If a caller only passes
    a predicted class and confidence, estimate the opposite-class score for the
    binary classifier
...
def postprocess_prediction(
    ...
    """Convert model output into collection priority

    The model label stays intact. Routing uses the needs_collection risk score:
    low risk becomes low priority, high risk becomes urgent, and the uncertain
    middle band goes to review
